<template>
    <div class="reservation-form p-4">
      ...
      <div v-else-if="step === 2">
        ...
        <div class="mb-4 p-4 border rounded shadow-md">
          <p class="font-medium">Météo du jour à {{ city }} :</p>
          <div v-if="todayLoading">
            Chargement de la météo du jour...
          </div>
          <div v-else-if="todayForecast">
            <p class="capitalize">{{ todayForecast.description }}</p>
            <p class="text-2xl">{{ todayForecast.temp }}°C</p>
          </div>
          <div v-else>
  ...
  const getTodayNoumea = () => {
    ...
    const noumeaOffset = 11; // Nouméa est généralement UTC+11
    const nowNoumea = new Date(nowUTC + noumeaOffset * 3600000);
    return nowNoumea.toISOString().split('T')[0];
  };
  ...
  // Récupération de la prévision météo pour la période de réservation (on se base sur la date de début)
  ...
  // Au montage, récupérer dès le début la météo du jour.
  fetchTodayWeather();
  ...
  // Vérification que le formulaire est prêt à être soumis
  const canSubmit = computed(() => {
    return lastName.value && selectedRoomType.value && personCount.value > 0;
  });
  
  // Soumission finale du formulaire avec validation du nombre de personnes en fonction du type
